    ...
    return (
        <div className="glass-card" style={{ width: '100%', maxWidth: '440px', padding: '40px' }}>
            <div style={{ display: 'flex', background: 'rgba(255, 255, 255, 0.05)', borderRadius: '12px', padding: '4px', marginBottom: '32px' }}>
                <button onClick={() => setMode('login')} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: mode === 'login' ? 'var(--primary)' : 'transparent', color: 'white', cursor: 'pointer', fontWeight: '600' }}>账号登录</button>
                <button onClick={() => setMode('signup')} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: mode === 'signup' ? 'var(--primary)' : 'transparent', color: 'white', cursor: 'pointer', fontWeight: '600' }}>新用户注册</button>
            </div>
            ...
            <form action={mode === 'login' ? login : signup}>
                ...
                <button type="submit" className="premium-btn" style={{ width: '100%', justifyContent: 'center', height: '48px', background: mode === 'signup' ? 'linear-gradient(135deg, #0ea5e9 0%, #2563eb 100%)' : 'var(--primary)' }}>
